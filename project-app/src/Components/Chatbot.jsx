import { useEffect, useState } from "react"
import logo from '/StackQAI-new/Frontend/project-app/src/icons8-dots-loading.gif'
import Output from "./Output"

export default function Chatbot({ answer, loader }) {
    const [chatLoader, setChatLoader] = useState(loader)

    useEffect(() => {
        if (answer != "") {
            setChatLoader(false)
        }
    }, [chatLoader, answer])

    return (
        <>
            {
                chatLoader == true ?
                    <div>
                        <img src={logo}></img>
                    </div> :
                    <div className="mt-[4px] flex">
                        <img src="data:image/png;base64," className="h-[35px] w-[35px]" />
                        <div className="font-normal text-sm pl-2 pr-2 text-slate-900 lg:text-lg">
                            <Output text={answer}></Output>
                        </div>
                    </div>
            }

        </>
    )
}